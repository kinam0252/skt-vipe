...
def project_points(
    ...
    """
    Projects 3D points onto a 2D canvas using given intrinsics and pose,
    now with a simple depth buffer (z-buffer) to ensure correct visibility.
    """
    assert pose.shape == (), "Only single pose is supported"
    ...
    if color is not None:
        ...
        color = np.tile(np.array([[0, 255, 0]], dtype=np.uint8), (len(uv), 1))

    # --- Z-buffer: Only draw nearest point per pixel ---
